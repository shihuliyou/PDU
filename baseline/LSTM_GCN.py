...
class LSTM_GCN(nn.Module):
    def __init__(self, input_dim, hidden_dim, gcn_hidden_dim, output_dim, num_layers,data=None,adj=None,adj_index=None):
        ...
        # GCN层
        self.gcn1 = pyg_nn.GCNConv(hidden_dim, gcn_hidden_dim)
    ...
    def forward(self, x):
        ...
        # x的形状：[节点数, 时间步, 特征数]
        # edge_index表示节点间的连接，用于GCN
        # 1. LSTM层：逐节点处理时间序列
        # 调整维度以适应LSTM输入 [节点数, 时间步, 特征数] => [节点数, 时间步, 隐藏维度]
        batch_size, time_steps, _ = x.size()
        x, _ = self.lstm(x)  # 输出shape为 [节点数, 时间步, 隐藏维度]
        # 取LSTM的最后一个时间步的输出，形状变为 [节点数, 隐藏维度]
        x = x[:, -1, :]
        # 2. GCN层：处理节点之间的关系

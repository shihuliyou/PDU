...
def get_loss(prediction, truth_price, base_price, mask, batch_size, alpha):
    device = prediction.device
    base_price = base_price.view(-1, 1)
    prediction = prediction.view(-1, 1)
    all_one = torch.ones(batch_size, 1, dtype=torch.float32).to(device)
    return_ratio = prediction
    ground_truth = torch.div(torch.sub(truth_price, base_price), base_price)
...
class StockMixer(nn.Module):
    def __init__(self, stocks, time_steps, channels, market, scaler):
    ...
    def forward(self, inputs):
        inputs = inputs['x']  # [B, T, N, C]
        inputs = inputs.squeeze(0).permute(1, 0, 2)  # [N, T, C]
        ...
        z = self.time_fc_(z)
        output = y + z  # [N, T]
        output = output.permute(1, 0).unsqueeze(0).unsqueeze(-1)
        return output

    def predict(self, data):
        with torch.no_grad():
            pred_ratio = self.forward(data).squeeze()
            base_price = data['x'][0, -1, :, -1].squeeze()
            pred_price = torch.mul(base_price, pred_ratio) + base_price
            pred_price = pred_price.view(1, 1, -1, 1)
            return pred_price

    def cal_train_loss(self, data):
        pred_ratio = self.forward(data)  # [B, T, N, C]

        pred_ratio = pred_ratio.view(-1, 1)
        truth_price = self.scaler.inverse_transform(data['y']).view(-1, 1)
        mask = data['mask'].min(dim=1)[0].view(-1, 1)
        data['x'] = self.scaler.inverse_transform(data['x'])

        loss = get_loss(pred_ratio, truth_price, data['x'][0, -1, :, -1], mask, self.stocks, 0.1)[0]
        return loss
...
if __name__ == '__main__':
    ...
    model = StockMixer(100, 16, 9, 20, scaler)
    data = {
        'x': torch.randn(1, 16, 100, 9),
        'y': torch.randn(1, 1, 100),
        'mask': torch.randn(1, 1, 100)
    }
    loss = model.cal_train_loss(data)
    print(loss)

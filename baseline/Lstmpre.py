...
class StockLSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # 通过 LSTM 层
        lstm_out, (hn, cn) = self.lstm(x)  # lstm_out 形状: (batch_size, 4, 64)
        # 取最后一个时间步的输出
        last_out = lstm_out[:, -1, :]  # 取最后一个时间步，形状: (batch_size, 64)
        # 全连接层
...
# # 示例输入
# batch_size = 1026  # 样本数为1026
# input_tensor = torch.randn(batch_size, 4, 9)  # 随机生成输入数据，形状为 (1026, 4, 9)
#
# # 创建模型
...
#
# print(output.shape)  # 应该输出 (1026, 1)
